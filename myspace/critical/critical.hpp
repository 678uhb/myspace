...
public:
  ...
  _CriticalImpl(Targs &&... args) : Hold(forward<Targs>(args)...) {}
};

template <class Hold, class Mtx, class Cond>
class _CriticalImpl<
    Hold, Mtx, Cond,
    typename std::enable_if<is_fundamental<Hold>::value, Hold>::type>
    : public Mtx, public Cond {
public:
  template <class... Targs>
  _CriticalImpl(Targs &&... args) : _hold(forward<Targs>(args)...) {}
  ...
  bool operator<(const Hold &other) const { return _hold < other; }

  void operator--() { --_hold; }

  void operator++() { ++_hold; }

  operator const Hold &() const { return _hold; }
