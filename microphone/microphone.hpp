...
public:
  ...
  // open microphone
  Microphone(uint32_t no, const wav::Format &format, size_t voicelength,
...
private:
  static void CALLBACK callbackFunc(HWAVEIN hwavein, UINT uMsg,
                                    ...
                                    DWORD_PTR dwParam2);
  void prepareBufferProc(uint32_t no);
...
inline void Microphone::prepareBufferProc(uint32_t no) {
  try {

    MYSPACE_DEFER(stop_ = true; cond_.notify_all());
    auto n =
        ::waveInOpen(&wave_in_, (uint32_t)no, &format_,
    ...
    for (size_t i = 0; i < buff_count; ++i) {
      ...
      buff->dwBufferLength = (DWORD)voicelength_;
...
inline Microphone::Microphone(
    uint32_t no, const wav::Format &t_format, size_t voicelength,
    std::function<void(std::string &&voice)> callback) noexcept(false)
